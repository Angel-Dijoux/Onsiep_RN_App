...
export const OnisepProvider = ({children}) => {
    ...
    const GetFormation = async() => {
        ...
        axios.get("https://api.opendata.onisep.fr/api/1.0/dataset/5fa591127f501/search?size=5000", {
            headers: { 
    ...
    const filterFormation = (value) => {
        if(value){
            const newData = filterData.filter((item) => {
                const itemData = item.libelle_type_formation ? item.libelle_type_formation.toUpperCase() : ''.toUpperCase()
